...
export default function AppShell() {
  ...
  const collectionsActive =
    pathname.startsWith('/users') && (!userId || !pathname.startsWith(`/users/${userId}`))
          ...
          <div className="ml-auto flex items-center gap-3">
            {token ? (
              ...
              <>
                ...
                <NavLink to="/register" className="text-sm font-medium text-indigo-600 hover:text-indigo-800 transition-colors">Register</NavLink>
              </>
            )}
          </div>
        </div>
